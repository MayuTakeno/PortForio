class Public::ChatsController < ApplicationController
  ...
  def index
    @chat = Chat.new
    @chats = Chat.includes(:employee).page(params[:page]).per(10).order(created_at: :desc)
    if params[:word].present?
      @chats = Chat.where("title LIKE?", "%#{params[:word]}%").page(params[:page]).per(10).order(created_at: :desc)
    end
  end
