class Public::BlogsController < ApplicationController
  ...
  def index
    @blogs = Blog.includes(:employee).page(params[:page]).per(10).order(created_at: :desc)
    if params[:word].present?
      @blogs = Blog.where("title LIKE?", "%#{params[:word]}%").page(params[:page]).per(10).order(created_at: :desc)
    end
    @blog = Blog.new
  end
